...
- Name: diagnose.spec.ts >> 진단 페이지 — UI 인터랙션 >> 이미지 업로드 영역 존재
- Location: e2e\diagnose.spec.ts:71:7
...
Locator: locator('text=불량 사진 업로드').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=불량 사진 업로드').first()
