...
// App Store 6.9" 스크린샷 5장 생성 (ios-screenshots-v1_4-en-v1 — 로케일 파라미터화 --locale=ko|en).
...
// --locale=ko|en (기본 ko) 로 UI 로케일 선택 — localStorage['molddoctor_locale']로 강제 고정.
...
const LOCALES = {
  ko: {
    browserLocale: 'ko-KR',
    outDir: path.join(process.cwd(), 'store-assets', 'ios-v1.4-ko'),
    t: {
      login: '로그인',
      typeSelect: '불량유형 직접 선택',
      historyHeading: /추정 기록 \(\d+\)/,
      restore: '다시 보기',
    },
  },
  en: {
    browserLocale: 'en-US',
    outDir: path.join(process.cwd(), 'store-assets', 'ios-v1.4-en'),
    t: {
      login: 'Log in',
      typeSelect: 'Choose defect type',
      historyHeading: /Diagnosis history \(\d+\)/,
      restore: 'Restore',
    },
  },
};
...
function parseLocale() {
  ...
  const key = arg ? arg.slice('--locale='.length).trim() : 'ko';
  if (!LOCALES[key]) {
    console.error(`--locale 값이 올바르지 않습니다: '${key}' (지원: ${Object.keys(LOCALES).join(', ')})`);
    process.exit(1);
  }
  return key;
}

async function loginViaModal(page, L) {
  ...
  return [
    ...
    },
    {
      ...
      capture: async (page) => {
        ...
        await page.waitForSelector(`text=${L.t.typeSelect}`, { timeout: 15000 });
        await page.screenshot({ path: path.join(outDir, '02-diagnose-input.png'), fullPage: false });
      },
    },
    {
      n: 3,
      ...
      needsLogin: true,
      capture: async (page) => {
        // 기존 진단 이력 1건을 "다시 보기/Restore"로 연다 — 신규 진단 실행·크레딧 소모 없음.
        await page.goto(`${BASE_URL}/account`, { waitUntil: 'networkidle' });
        const historyHeading = page.getByRole('button', { name: L.t.historyHeading });
        ...
        const restoreButton = firstCard.getByRole('button', { name: L.t.restore });
        ...
        await page.screenshot({ path: path.join(outDir, '03-diagnose-result.png'), fullPage: false });
      },
    },
    {
      n: 4,
      ...
      capture: async (page) => {
        ...
        await page.waitForSelector('h1', { timeout: 15000 });
        await page.screenshot({ path: path.join(outDir, '04-tryout.png'), fullPage: false });
      },
    },
    {
      n: 5,
      ...
      capture: async (page) => {
        ...
        await page.waitForSelector('h1', { timeout: 15000 });
        await page.screenshot({ path: path.join(outDir, '05-tools.png'), fullPage: false });
      },
    },
  ];
}
...
async function main() {
  const localeKey = parseLocale();
  const L = LOCALES[localeKey];
  const outDir = L.outDir;
  await mkdir(outDir, { recursive: true });
  log(`로케일: ${localeKey} (${L.browserLocale}) → ${outDir}`);

  const TARGETS = buildTargets(L, outDir);
  ...
  const context = await browser.newContext({
    ...
    isMobile: true,
    hasTouch: true,
    locale: L.browserLocale,
  });
  // localStorage 값은 아직 page가 없어 addInitScript로 모든 페이지에 선주입 — locale 강제 고정.
  await context.addInitScript((localeValue) => {
    try { localStorage.setItem('molddoctor_locale', localeValue); } catch { /* ignore */ }
  }, localeKey);
  ...
  if (skipped.length > 0) {
    ...
    log(`실행: SCREENSHOT_EMAIL=... SCREENSHOT_PASSWORD=... node scripts/store-screenshots.mjs --locale=${localeKey} --only=${nums}`);
  } else {
