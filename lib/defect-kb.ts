...
★ 가수분해 카브아웃: 수지의 moisture mode가 hydrolysis 또는 both(PC·PBT·PA 등 — resin mechanism 참조)이고 건조 조건이 권장 미달로 확인되면, 이 건은 외관 문제가 아니라 물성 문제다. (1) 은줄이 보이지 않는 로트·샷도 물성 손상 가능성을 명시하고, 해당 생산분 격리 + 물성 확인(인장/충격) 후 출하 판단을 권고하라. "외관 양품 = 출하 가능"이라고 답하지 마라 — 가수분해는 재건조로 회복되지 않고, 은줄 없이도 조기 파손할 수 있다. (2) severity는 입력에 기능·강도 요구가 명시된 부품일 때만 high로 판정하라(파단 강도 부족 기준). 용도가 불명이거나 외관 전용이면 medium을 유지하되, 권고문에 "기능·강도 요구 부품이라면 high에 해당하며 물성 확인이 필수"임을 명시하라. GF 함량만으로 기능부품이라고 단정하지 마라. (3) 건조 조건이 권장을 충족하면 이 카브아웃은 적용하지 않는다.`,
